...
const DetailProduct = () => {
  const { products, backendurl, userData, token, replies } =
    ...
    const fetchComments = async () => {
      ...
      try {
        const res = await axios.get(
          `${backendurl}/api/comment/get-comments-by-product/${prID}`
        );
        setAllComments(res.data);
        if (userData) {
          const userExistingComment = res.data.find(
            (comment) => comment.userId === userData._id
          );
    ...
    try {
      ...
      if (userComment) {
        res = await axios.post(
          `${backendurl}/api/comment/update-comment`,
          {
            userId: userData._id,
            productId: prID,
            text: commentText,
        ...
        <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg w-auto mt-8">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="ml-0 md:ml-6 mt-4 md:mt-0 w-full md:w-1/2">
              ...
              <p className="text-md md:text-xl text-gray-700 mt-2">
                Giá: {pr.price} ₫
              </p>
              <div className="mt-4">
              ...
              <div className="mt-6">
                ...
                <table className="w-full mt-4 table-auto">
                  ...
                  <tbody>
                    {pr.specifications &&
                    Object.keys(pr.specifications).length > 0 ? (
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 border-t pt-2">
              {userData ? (
                ...
                >
                  <button className="ml-3 mt-2 mb-2">
                      ...
                      {replies
                        ...
                        .map((reply) => (
                          <div key={reply._id} className="text-gray-600 mb-1">
                            <strong className="ml-3">{"Admin"}:</strong>{" "}
                            {reply.text}
                          </div>
                        ))}
                    </div>
