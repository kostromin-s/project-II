...
const AppContextProvider = (props) => {
  ...
  const addMessages = async (newMsgs) => {
    try {
      const a = await axios.post(
        backendurl + "/api/user/ask-and-save-groq",
        { message: newMsgs },
        { headers: { token } }
      );
  ...
  const getMessages = async () => {
    try {
      const x = await axios.get(backendurl + "/api/user/get-conversation", {
        headers: { token },
      });
  ...
  const getComments = async () => {
    try {
      const { data } = await axios.get(
        backendurl + "/api/comment/get-comments",
        { headers: { token } }
      );
  ...
  const deleteUser = async () => {
    try {
      ...
      toast.success(
        "Delete user successfully. Let's navigate to home page without sign in"
      );
    } catch (error) {
      console.log(error);
      toast.error(error.message);
  ...
  const getRepliesByUser = async () => {
    try {
      const { data } = await axios.get(
        backendurl + "/api/user/get-my-replies",
        { headers: { token } }
      );
  ...
  const getAllReplies = async () => {
    try {
      const { data } = await axios.get(
        backendurl + "/api/user/get-all-replies",
        { headers: { token } }
      );
      if (!data) toast.warn("No replies yet or Error connect server");
  ...
  const markOneAsRead = async (notificationId) => {
    try {
      await axios.post(
        backendurl + "/api/user/mark-one-as-read",
        { notificationId },
        { headers: { token } }
      );
    } catch (error) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        await Promise.all([getProductsData(), getAllReplies()]);
      } catch (error) {
