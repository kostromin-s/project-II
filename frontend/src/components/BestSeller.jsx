...
const BestSeller = () => {
  ...
  useEffect(() => {
    const fetchLaptops = async () => {
      try {
        ...
        const laptops = res.data.products.filter(
          (p) => p.category === "Laptop"
        );
  ...
  return (
    <div className="mt-8 py-8 px-4 relative">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Ưu đãi tốt nhất</h2>
      ...
      >
        {bestSellingLaptops.length === 0 ? (
          <p className="text-gray-500 mt-10">Không tìm thấy laptop bán chạy.</p>
        ) : (
          bestSellingLaptops.map((item) => (
            ...
            >
              ...
              />
              <p className="font-semibold truncate whitespace-nowrap overflow-hidden">
                {item.name}
              </p>
              ...
              <p className="text-red-600 font-bold mt-4">
                {item.price} ₫
                <span className="text-purple-600 text-xs"> - bán chạy</span>
              </p>
            </div>
          ))
        )}
